...
function donateReducer(state, action) {
  switch (action.type) {
    case 'FETCH_START': return { ...state, loading: true };
    case 'FETCH_SUCCESS': return { data: action.data, loading: false, error: "" };
...
export default function DonateModal({ isOpen, onClose }) {
  const [{ data, loading, error }, dispatch] = useReducer(donateReducer, { data: null, loading: false, error: "" });
  const modalRef = useRef(null);

  const isOpenRef = useRef(false);
  useEffect(function fetchDonateData() {
    if (!isOpen) { isOpenRef.current = false; dispatch({ type: 'RESET' }); return; }
    if (isOpenRef.current) return;
    isOpenRef.current = true;
    const controller = new AbortController();
    dispatch({ type: 'FETCH_START' });
    function onResponse(res) { if (!res.ok) throw new Error(`HTTP ${res.status}`); return res.json(); }
    function onJson(json) { if (!controller.signal.aborted) dispatch({ type: 'FETCH_SUCCESS', data: json }); }
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <button type="button" className="absolute inset-0 bg-black/30 backdrop-blur-sm" onClick={onClose} aria-label="Close modal" />
      <div
        ref={modalRef}
