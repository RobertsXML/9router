...
const AWS_SSO_CACHE_PATH = join(homedir(), ".aws/sso/cache");

// Hoist I/O to module scope with TTL cache — SSO cache and profile files change rarely
let cachedKiroResult = null;
let cachedKiroTime = 0;
const KIRO_CACHE_TTL_MS = 60_000;

async function discoverKiroToken() {
  const now = Date.now();
  if (cachedKiroResult && now - cachedKiroTime < KIRO_CACHE_TTL_MS) {
    return cachedKiroResult;
  }

  const cachePath = AWS_SSO_CACHE_PATH;
  let files;
  try {
  ...
  if (!refreshToken) {
    for (const file of files) {
      ...
      try {
        // eslint-disable-next-line no-await-in-loop -- sequential: find first matching token file, early break
  ...
  for (const profilePath of kiroProfilePaths) {
    try {
      // eslint-disable-next-line no-await-in-loop -- sequential: find first profile with arn, early break
  ...
  const result = { found: true, refreshToken, source: foundFile, clientId, clientSecret, region, authMethod, profileArn };
  cachedKiroResult = result;
  cachedKiroTime = now;
  return result;
}
...
export async function GET() {
  try {
    return NextResponse.json(await discoverKiroToken());
  } catch (error) {
