...
const statusReducer = (state, action) => {
  switch (action.type) {
    case 'setJcodeStatus': return { ...state, jcodeStatus: action.value };
    case 'setCheckingJcode': return { ...state, checkingJcode: action.value };
...
const formReducer = (state, action) => {
  switch (action.type) {
    ...
    case 'resetForm': return { ...state, selectedModel: "", selectedApiKey: "" };
    default: return state;
  }
};
...
function ToolHeader({ tool, configStatus, isExpanded, handleToggle }) {
  return (
...
function JcodeStatusIndicator({ checkingJcode, jcodeStatus, setShowManualConfigModal }) {
  return (
    <>
      {checkingJcode && (
...
function JcodeConfigFields({
  tool, jcodeStatus, form, dispatch, hasActiveProviders,
    ...
    <div className="flex flex-col gap-2">
      ...
      <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr_auto] sm:items-center sm:gap-2">
        ...
        <div className="relative w-full min-w-0">
          <input type="text" value={form.selectedModel} onChange={(e) => dispatch({ type: 'setSelectedModel', value: e.target.value })} placeholder="cc/claude-opus-4-7" aria-label="Model ID" className="w-full min-w-0 pl-2 pr-7 py-2 bg-surface rounded border border-border text-xs focus:outline-none focus:ring-1 focus:ring-primary/50 sm:py-1.5" />
      ...
      <div className="flex flex-col gap-1 p-3 bg-blue-500/5 border border-blue-500/20 rounded-lg">
        ...
        <code className="text-xs font-mono text-text-muted">jcode --provider-profile 9router --model {form.selectedModel || "cc/claude-opus-4-7"}</code>
      </div>
    </div>
  );
}

function JcodeActionButtons({ handleApplySettings, handleResetSettings, applying, restoring, selectedModel, jcodeStatus, setShowManualConfigModal }) {
  return (
...
export default function JcodeToolCard({
  ...
  const [status, statusDispatch] = useReducer(statusReducer, {
    jcodeStatus: initialStatus || null,
    checkingJcode: false,
    applying: false,
    restoring: false,
    message: null,
  ...
  const [form, formDispatch] = useReducer(formReducer, {
    selectedApiKey: (() => {
      const envKey = initialStatus?.envApiKey;
      if (envKey && apiKeys?.some((k) => k.key === envKey)) return envKey;
      return "";
    })(),
    selectedModel: initialStatus?.config?.providers?.["9router"]?.default_model || "",
    customBaseUrl: "",
    modalOpen: false,
  ...
  const { jcodeStatus, checkingJcode, applying, restoring, message } = status;
  ...
  }, [jcodeStatus, tunnelPublicUrl, tailscaleUrl]);
  ...
  const checkJcodeStatus = useCallback(async () => {
    statusDispatch({ type: 'setCheckingJcode', value: true });
    try {
      const res = await fetch("/api/cli-tools/jcode-settings");
      const data = await res.json();
      statusDispatch({ type: 'setJcodeStatus', value: data });
    } catch (error) {
      statusDispatch({ type: 'setJcodeStatus', value: { installed: false, error: error.message } });
    } finally {
      statusDispatch({ type: 'setCheckingJcode', value: false });
    }
  }, []);
  ...
  const handleToggle = useCallback(() => {
    ...
    onToggle();
  }, [isExpanded, onToggle, jcodeStatus, checkJcodeStatus, fetchModelAliases]);
...
id = "${selectedModel || "cc/claude-opus-4-7"}"`;
  ...
  return (
    <Card padding="xs" className="overflow-hidden">
      <ToolHeader tool={tool} configStatus={configStatus} isExpanded={isExpanded} handleToggle={handleToggle} />

      {isExpanded && (
        <div className="mt-4 pt-4 border-t border-border flex flex-col gap-4">
          <JcodeStatusIndicator checkingJcode={checkingJcode} jcodeStatus={jcodeStatus} setShowManualConfigModal={(v) => formDispatch({ type: 'setShowManualConfigModal', value: v })} />

          {!checkingJcode && jcodeStatus?.installed && (
            <>
              <JcodeConfigFields
                tool={tool}
                jcodeStatus={jcodeStatus}
                form={form}
