  ...
  return (
    <div className="flex flex-col gap-2 mb-4">
      {notes.map((note, index) => {
        ...
        return (
          <div key={`${note.type}-${index}`} className={`flex items-start gap-3 p-3 rounded-lg border ${bgClass}`}>
...
function GuideSteps({ tool, cloudEnabled, tunnelEnabled, replaceVars, handleCopy, copiedField, renderApiKeySelector, renderModelSelector }) {
...
export default function DefaultToolCard({ toolId, tool, isExpanded, onToggle, baseUrl, apiKeys, activeProviders = EMPTY_PROVIDERS, cloudEnabled = false, tunnelEnabled = false }) {
      ...
      >
        <div className="flex items-center gap-3">
          <div className="size-8 rounded-lg flex items-center justify-center shrink-0">
            <ToolIcon toolId={toolId} tool={tool} />
          </div>
      ...
      {isExpanded && (
        <div className="mt-6 pt-6 border-t border-border">
          <GuideSteps
            ...
            renderApiKeySelector={renderApiKeySelector}
