...
function reducer(state, action) {
  switch (action.type) {
    ...
    case "setActiveModel": return { ...state, activeModel: action.value };
    case "reset": return { ...initialState };
    default: return state;
  }
}

function ToolHeader({ tool, isExpanded, onToggle, configStatus }) {
  return (
...
function InstallGuideSection({ showInstallGuide, dispatch }) {
  return (
...
function ModelChips({ selectedModels, activeModel, onChipClick, onRemove }) {
  return (
      ...
      {selectedModels.length === 0 ? (
        ...
        selectedModels.map((model) => (
          <button
            type="button"
            key={model}
            onClick={() => onChipClick(model)}
            onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); onChipClick(model); } }}
          ...
          >
            ...
            {model}
            {/* eslint-disable-next-line react-doctor/prefer-tag-over-role -- span with role=button avoids nested interactive elements inside button */}
            <span
              role="button"
              tabIndex={0}
              onClick={(e) => {
              ...
              onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.stopPropagation(); onRemove(model); } }}
...
function ConfigForm({
  status, customBaseUrl, selectedApiKey, selectedModel, selectedModels, activeModel, subagentModel,
  dispatch, getEffectiveBaseUrl, handleModelChipClick, handleRemoveModel,
  activeProviders, apiKeys, tunnelEnabled, tunnelPublicUrl, tailscaleEnabled, tailscaleUrl,
  tool, cloudEnabled
}) {
  return (
    <div className="flex flex-col gap-2">
      ...
      <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr] sm:items-start sm:gap-2">
        ...
        <div className="flex-1 flex flex-col gap-2">
          <ModelChips selectedModels={selectedModels} activeModel={activeModel} onChipClick={handleModelChipClick} onRemove={handleRemoveModel} />
          <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr_auto] sm:items-center sm:gap-2">
      ...
      <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr_auto] sm:items-center sm:gap-2">
        ...
        <input
          type="text"
          value={subagentModel}
          onChange={(e) => dispatch({ type: "setSubagentModel", value: e.target.value })}
        ...
        <button
          type="button"
          onClick={() => dispatch({ type: "setSubagentModalOpen", value: true })}
        ...
        {subagentModel && (
          <button
            type="button"
            onClick={() => dispatch({ type: "setSubagentModel", value: "" })}
...
function ActionButtons({ onApply, onReset, applying, restoring, has9Router, selectedModels, message, dispatch }) {
  return (
    <>
      {message && (
      ...
      <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
        <Button variant="primary" size="sm" onClick={onApply} disabled={selectedModels.length === 0} loading={applying}>
          <span className="material-symbols-outlined text-[14px] mr-1">save</span>Apply
        </Button>
        <Button variant="outline" size="sm" onClick={onReset} disabled={!has9Router} loading={restoring}>
...
function OpenCodeExpandedContent({ checking, status, showInstallGuide, dispatch, customBaseUrl, selectedApiKey, selectedModel, selectedModels, activeModel, subagentModel, getEffectiveBaseUrl, handleModelChipClick, handleRemoveModel, activeProviders, apiKeys, tunnelEnabled, tunnelPublicUrl, tailscaleEnabled, tailscaleUrl, tool, cloudEnabled, handleApply, handleReset, applying, restoring, message }) {
  return (
    <div className="mt-4 pt-4 border-t border-border flex flex-col gap-4">
      ...
      {!checking && status && !status.installed && (
        <InstallGuideSection showInstallGuide={showInstallGuide} dispatch={dispatch} />
      )}
      ...
      {!checking && status?.installed && (
        <>
          <ConfigForm
            ...
            handleModelChipClick={handleModelChipClick}
...
function ToolModals({ modalOpen, subagentModalOpen, showManualConfigModal, dispatch, saveModels, selectedModelsRef, selectedModels, activeModel, subagentModel, activeProviders, modelAliases, getManualConfigs }) {
  return (
    <>
      <ModelSelectModal
        isOpen={modalOpen}
        ...
        onSelect={(model) => {
          if (!selectedModels.includes(model.value)) {
            dispatch({ type: "setSelectedModels", value: [...selectedModels, model.value] });
            if (!activeModel) dispatch({ type: "setActiveModel", value: model.value });
          }
        }}
        onDeselect={(model) => {
          const remaining = selectedModels.filter(m => m !== model.value);
          dispatch({ type: "setSelectedModels", value: remaining });
          if (activeModel === model.value) {
            dispatch({ type: "setActiveModel", value: remaining[0] || "" });
          }
        }}
        selectedModel={null}
        activeProviders={activeProviders}
...
export default function OpenCodeToolCard({ tool, isExpanded, onToggle, baseUrl, apiKeys, activeProviders, cloudEnabled, initialStatus, tunnelEnabled, tunnelPublicUrl, tailscaleEnabled, tailscaleUrl }) {
  const [state, dispatch] = useReducer(reducer, null, () => ({ ...initialState, status: initialStatus || null }));
  const { status, checking, applying, restoring, message, showInstallGuide, selectedApiKey, selectedModel, subagentModel, modalOpen, subagentModalOpen, modelAliases, showManualConfigModal, customBaseUrl, selectedModels, activeModel } = state;
  ...
  const checkStatus = useCallback(async () => {
    ...
    try {
      ...
      if (!hasInitializedFromStatus.current) {
        hasInitializedFromStatus.current = true;
        if (data?.opencode?.models) {
          dispatch({ type: "setSelectedModels", value: data.opencode.models });
        }
        if (data?.opencode?.activeModel) {
          dispatch({ type: "setActiveModel", value: data.opencode.activeModel });
        }
        if (data?.config?.agent?.explorer?.model?.startsWith("9router/")) {
          dispatch({ type: "setSubagentModel", value: data.config.agent.explorer.model.replace("9router/", "") });
        }
      }
    } catch (error) {
    ...
    if (!isExpanded && !fetchedForExpansion.current) {
      ...
      if (!status) {
        ...
        fetch("/api/cli-tools/opencode-settings")
          ...
          .then((data) => {
            dispatch({ type: "setStatus", value: data });
            if (data?.opencode?.models) dispatch({ type: "setSelectedModels", value: data.opencode.models });
            if (data?.opencode?.activeModel) dispatch({ type: "setActiveModel", value: data.opencode.activeModel });
            if (data?.config?.agent?.explorer?.model?.startsWith("9router/")) {
              dispatch({ type: "setSubagentModel", value: data.config.agent.explorer.model.replace("9router/", "") });
            }
          })
  ...
  const saveModels = useCallback(async (models) => {
    try {
  ...
  }, [selectedApiKey, cloudEnabled, activeModel, getEffectiveBaseUrl, subagentModel]);
  ...
  }, [selectedApiKey, cloudEnabled, selectedModels, activeModel, subagentModel, getEffectiveBaseUrl, checkStatus]);
  ...
  const handleReset = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        dispatch({ type: "setActiveModel", value: "" });
        checkStatus();
      } else {
  ...
  }, [selectedApiKey, cloudEnabled, selectedModels, activeModel, subagentModel, getEffectiveBaseUrl]);

  const handleModelChipClick = useCallback(async (model) => {
    if (model === activeModel) {
      ...
      dispatch({ type: "setActiveModel", value: model });
    }
  }, [activeModel, checkStatus]);

  const handleRemoveModel = useCallback(async (model) => {
    try {
      ...
      if (res.ok) {
        ...
        dispatch({ type: "setSelectedModels", value: newModels });
  ...
  }, [selectedModels, activeModel, checkStatus]);

  return (
    <Card padding="xs" className="overflow-hidden">
      <ToolHeader tool={tool} isExpanded={isExpanded} onToggle={handleToggle} configStatus={configStatus} />

      {isExpanded && (
        <OpenCodeExpandedContent
          checking={checking}
          status={status}
      ...
      <ToolModals
        modalOpen={modalOpen}
        subagentModalOpen={subagentModalOpen}
        showManualConfigModal={showManualConfigModal}
        dispatch={dispatch}
        saveModels={saveModels}
        selectedModelsRef={selectedModelsRef}
        selectedModels={selectedModels}
        activeModel={activeModel}
        subagentModel={subagentModel}
        activeProviders={activeProviders}
        modelAliases={modelAliases}
        getManualConfigs={getManualConfigs}
      />
    </Card>
  );
}
