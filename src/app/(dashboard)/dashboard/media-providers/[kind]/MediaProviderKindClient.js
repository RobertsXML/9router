...
function ProviderStatus({ isNoAuth, allDisabled, total, connected, error }) {
          ...
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                ...
                <ProviderStatus isNoAuth={isNoAuth} allDisabled={allDisabled} total={total} connected={connected} error={error} />
              </div>
            </div>
          </div>
          {total > 0 && (
            ...
            <div className="flex min-w-0 items-center gap-3">
              ...
              <div className="flex flex-wrap items-center gap-1 sm:shrink-0">
                {combo.models.slice(0, 6).map((entry, i) => {
                  ...
                  return (
                    <div key={`${entry}-${pid}`} title={p?.name || entry} className="size-5 rounded flex items-center justify-center" style={{ backgroundColor: `${(p?.color ?? "#888")}15` }}>
                      <ProviderIcon
...
export default function MediaProviderKindClient({ initialConnections, initialCustomNodes, initialCombos }) {
  ...
  // webSearch/webFetch redirected to /web via middleware
  const kindConfig = MEDIA_PROVIDER_KINDS.find((k) => k.id === kind);
  const isEmbedding = kind === "embedding";
  const supportsCombo = COMBO_KINDS.has(kind);

  if (!kindConfig) return null; // parent handles notFound
    ...
    <div className="flex flex-col gap-6">
      {(isEmbedding || supportsCombo) && (
        <div className="flex items-center justify-end gap-2">
          {supportsCombo && (
            <Button type="button" size="sm" icon="add" onClick={handleCreateCombo}>Create Combo</Button>
          )}
          {isEmbedding && (
            <Button type="button" size="sm" icon="add" onClick={() => setShowAddCustomEmbedding(true)}>
