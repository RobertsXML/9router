...
export default function MediaProviderDetailClient({ initialCustomNode }) {
  ...
  const [customState, setCustomState] = useState({ node: initialCustomNode, loading: false });
  ...
  const provider = isCustom
    ? (customState.node ? { id, name: customState.node.name || "Custom Embedding", color: "#6366F1", textIcon: "CE" } : null)
    : builtInProvider;

  if (!isCustom && !builtInProvider) return notFound();
  if (isCustom && !customState.node) return notFound();
    ...
    <div className="flex flex-col gap-8">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-4">
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center gap-1.5 mt-1 flex-wrap">
              {isCustom && <Badge variant="default" size="sm">Custom · {customState.node?.prefix}</Badge>}
              {kinds.map((k) => (
      ...
      {kind !== "tts" && kind !== "webSearch" && kind !== "webFetch" && (
        <ModelsCard
          providerId={id}
          kindFilter={kind}
          providerAliasOverride={isCustom ? customState.node?.prefix : undefined}
        />
      )}
      ...
      {/* Example — per kind */}
      {kind === "embedding" && (
        <EmbeddingExampleCard providerId={id} customAlias={customState.node?.prefix} />
      )}
      ...
      {isCustom && (
        <AddCustomEmbeddingModal
          ...
          node={customState.node}
          onClose={() => setShowEditModal(false)}
          onSaved={(updated) => {
            setCustomState((s) => ({ ...s, node: updated }));
