...
function encodeRequest(messages, modelName, tools = [], reasoningEffort = null, forceAgentMode = false) {
  ...
  for (let i = 0; i < messages.length; i++) {
    ...
    if (msg?.role === "assistant" && hasToolCalls && hasToolResults) {
      ...
      for (const tr of msg.tool_results) { const id = tr?.tool_call_id; if (typeof id === "string") currentIds.add(id); }
      const nextIds = new Set();
      for (const tr of (nextMsg?.tool_results || [])) { const id = tr?.tool_call_id; if (typeof id === "string") nextIds.add(id); }
