...
// Cache: aliasOrId -> Map<modelId, modelEntry> for O(1) lookups
const _modelIndexCache = new Map();

function _getModelIndex(aliasOrId) {
  let idx = _modelIndexCache.get(aliasOrId);
  if (idx !== undefined) return idx;
  const models = PROVIDER_MODELS[aliasOrId];
  if (!models) { _modelIndexCache.set(aliasOrId, null); return null; }
  idx = new Map(models.map(m => [m.id, m]));
  _modelIndexCache.set(aliasOrId, idx);
  return idx;
}
...
export function isValidModel(aliasOrId, modelId, passthroughProviders = new Set()) {
  ...
  const idx = _getModelIndex(aliasOrId);
  return idx ? idx.has(modelId) : false;
}

export function findModelName(aliasOrId, modelId) {
  const idx = _getModelIndex(aliasOrId);
  if (!idx) return modelId;
...
export function getModelTargetFormat(aliasOrId, modelId) {
  const idx = _getModelIndex(aliasOrId);
  if (!idx) return null;
  return modelTargetFormat(idx.get(modelId));
}
...
export function getModelQuotaFamily(aliasOrId, modelId) {
  const idx = _getModelIndex(aliasOrId);
  return modelQuotaFamily(idx?.get(modelId));
}
...
export const OAUTH_ALIASES = Object.fromEntries(
  REGISTRY.reduce((acc, r) => { if (r.alias && r.alias !== r.id) acc.push([r.id, r.alias]); return acc; }, [])
);
